...
//                            teamSpaceBean.setAttachmentCount(jsonObject.getInt("AttachmentCount"));
            ...
            public void run() {
                try {
                    ...
                    if (returnjson.getInt("RetCode") == 0) {
                        ...
                        List<TeamSpaceBeanFile> list = new ArrayList<>();
                        for (int i = 0; i < jsonArray.length(); i++) {
                            JSONObject jsonObject = jsonArray.getJSONObject(i);
                            TeamSpaceBeanFile teamSpaceBeanFile = new TeamSpaceBeanFile();
    ...
    private void putInterface(int code, TeamSpaceInterfaceListener serviceInterfaceListener) {
        TeamSpaceInterfaceListener serviceInterfaceListener2 = hashMap.get(code);
